...
function Navigation() {
  return (
    <nav className="fixed left-1/2 top-3 z-20 flex min-h-14 w-[calc(100%-20px)] -translate-x-1/2 items-center justify-between gap-3 rounded-[28px] border border-white/60 bg-white/85 px-3 py-2 shadow-[0_20px_55px_rgba(16,54,35,0.12)] backdrop-blur-xl md:top-5 md:min-h-16 md:w-[min(1180px,calc(100%-32px))] md:rounded-full md:px-4">
      <Link className="flex items-center gap-2.5 font-extrabold text-[#123f2a]" to="/" aria-label="Cropixo home">
        <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-gradient-to-br from-[#1f6f43] to-[#67a36e] text-white">
      ...
      <div className="hidden items-center gap-2 text-sm font-semibold text-[#40524a] lg:flex">
        <Link className="rounded-full px-3.5 py-2 transition hover:bg-[#1f6f4315] hover:text-[#123f2a]" to="/products">Products</Link>
        <Link className="rounded-full px-3.5 py-2 transition hover:bg-[#1f6f4315] hover:text-[#123f2a]" to="/about">About</Link>
        <Link className="rounded-full px-3.5 py-2 transition hover:bg-[#1f6f4315] hover:text-[#123f2a]" to="/contact">Contact</Link>
      </div>
      ...
      <div className="mx-auto grid w-[min(1180px,100%)] gap-8 md:grid-cols-[1.2fr_0.8fr_0.8fr]">
        <div>
          <div className="mb-4 flex items-center gap-2.5 font-extrabold">
            <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white/12"><Leaf size={20} aria-hidden="true" /></span>
            <span>
              Import, Packed & Marketed by:
              <span className="block text-xl uppercase tracking-wide">Cropixo Innovation</span>
            </span>
          </div>
          <p className="max-w-md leading-7 text-white/75">Advanced fertilizer systems for cleaner, stronger harvests across greenhouse and field production.</p>
        </div>
        <div>
    ...
    <>
      <section className="relative grid min-h-[92svh] items-end px-5 pb-10 pt-28 text-white md:px-[clamp(20px,5vw,80px)] md:pb-14 md:pt-36">
        ...
        <div className="relative z-[1] max-w-[850px]">
          ...
          <h1 className="mb-6 max-w-[820px] text-[clamp(2.8rem,14vw,7rem)] font-bold leading-[0.94] tracking-normal">Advanced fertilizer systems for cleaner, stronger harvests.</h1>
          <p className="max-w-[620px] text-base leading-8 text-white/85 md:text-xl">Premium crop nutrition engineered for growers who demand precision, sustainability, and consistent field performance.</p>
          <div className="mt-8 flex flex-wrap gap-3.5">
            <Link className={buttonClass} to="/products">Explore products <ArrowRight size={18} /></Link>
            <Link className="inline-flex items-center justify-center rounded-full border border-white/50 bg-white/15 px-5 text-white backdrop-blur-md transition hover:-translate-y-0.5" to="/about">About Cropixo</Link>
          </div>
        </div>
      </section>
      ...
      <section className={`${sectionClass} grid items-center gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20`}>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {products.slice(0, 4).map((product) => (
            <Link className="rounded-3xl border border-[#1f6f4324] bg-white/80 p-5 shadow-[0_24px_70px_rgba(22,48,35,0.12)] transition hover:-translate-y-1" key={product.slug} to={`/products/${product.slug}`}>
    ...
    <>
      <section className={`${sectionClass} grid gap-10 pt-32 lg:grid-cols-2 lg:items-center`}>
        <div>
          ...
          <h1 className="mb-6 text-5xl font-bold leading-none text-[#15251c] md:text-7xl">Scientific fertility, designed with Dutch clarity.</h1>
          <p className={bodyClass}>Cropixo Nutrition develops high-grade fertilizers for protected cultivation, open fields, and distributors serving quality-focused growers.</p>
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {['24 crop protocols', '18 export markets', '0.3% batch variance'].map((metric) => (
              <span className="rounded-2xl border border-[#1f6f4324] bg-white p-4 text-[#65736b]" key={metric}>{metric}</span>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto grid w-[min(1180px,calc(100%-40px))] gap-5 pb-20 md:grid-cols-2">
        ...
        <article className="rounded-3xl border border-[#1f6f4324] bg-[#123f2a] p-8 text-white shadow-[0_24px_70px_rgba(22,48,35,0.12)]">
          ...
          <h2 className="mb-4 text-3xl font-bold leading-tight text-white md:text-4xl">Build a cleaner, more precise future for agriculture.</h2>
          <p className="text-base leading-8 text-white/80 md:text-lg"> Our Vision is to become a globally trusted leader in sustainable plant nutrition, recognized for pioneering agricultural innovation inspired by Dutch excellence in horticulture and precision farming. We envision a future where every farm benefits from smarter, eco-friendly fertilizer solutions that enhance yields, restore natural ecosystems, and contribute to global food security for generations to come.</p>
        </article>
      </section>
    </>
  )
  ...
  return (
    <section className={`${sectionClass} grid items-stretch gap-10 pt-32 lg:grid-cols-[0.9fr_1.1fr]`}>
      <div className="rounded-[32px] bg-[#123f2a] p-[clamp(28px,5vw,56px)] text-white">
        ...
        <p className="flex items-center gap-2.5 text-white/80"><Mail size={18} /> hello@cropixo.example</p>
        <p className="flex items-center gap-2.5 text-white/80"><Phone size={18} /> +31 70 000 0000</p>
      ...
      <form className="grid gap-4 rounded-3xl border border-[#1f6f4324] bg-white/80 p-[clamp(24px,4vw,42px)] shadow-[0_24px_70px_rgba(22,48,35,0.12)]" aria-label="Inquiry form">
        ...
        <label className="grid gap-2 font-bold text-[#123f2a]">Interest<select className="rounded-2xl border border-[#1f6f4324] bg-[#fbfdf9] px-4 py-3.5" defaultValue="product"><option value="product">Product inquiry</option><option value="dealer">Dealer partnership</option><option value="research">Research collaboration</option></select></label>
